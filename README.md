...
The remaining work to achieve the original fully real-time experience is substantial: validate the physical iPhone/glasses loop, read the new stage timings from a real session, keep a warm assistant process instead of starting one per question, attach camera images without a separate file-reading step, and investigate a documented subscription-compatible real-time media interface if OpenAI provides one. This prototype does not claim that interface exists.
