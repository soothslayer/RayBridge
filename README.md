...
The development app bundles the Node and Codex executables from the build machine. Claude Code and Hermes are discovered from their normal installations on the Mac. While a phone is connected with Claude Code selected, one Claude process stays open to serve that conversation; it ends when the phone disconnects, the conversation is reset, or the assistant or working folder changes. This build is for Apple Silicon and is ad-hoc signed, not notarized. It is not an installer ready for general distribution. Rebuild for another CPU architecture or older macOS deployment target. Do not run the source server and Mac app at the same time: both use ports 8844/8845.
...
4. Scan the Mac pairing QR with the iPhone Camera or open its pairing link. RayBridge validates the link, adds the Mac automatically, and selects it for the next session. Alternatively, open **Setup** and copy the link into RayBridge's pairing field. Pairing tokens are stored in the iPhone Keychain. RayBridge remembers the five most recently paired Macs; use **Setup → Saved Macs** to switch the Mac used by the next session. Under **Setup → Assistant**, use **Browse Mac folders** to navigate the selected Mac's home folder, or enter an absolute working-folder path manually. The preference is saved separately for each Mac and applied when the phone connects. Leave it blank to keep using the folder configured in the Mac app.
...
6. Select **Start RayBridge**, or say “Siri, start RayBridge.” Both methods announce that RayBridge is starting, and tapping **Stop RayBridge** announces that it is stopping. The Siri action opens the app and uses the same startup path as the button. RayBridge connects to your saved Mac, checks microphone/speech permissions, starts the glasses camera, waits for a usable image, and begins listening. Grant camera access in Meta AI if requested, then return to RayBridge. Wait for **Glasses camera connected**, ask a question, pause, and wait for the spoken answer. On subsequent launches, use either Start method.
...
**Setup → Camera and audio** makes the choice permanent in either direction. **This iPhone** skips the glasses check, the warning, and Meta registration entirely, which is the mode to use if you do not own the glasses. **Meta glasses** restores the default. Granting RayBridge iPhone camera access is requested the first time this mode starts. If iOS does not allow multitasking camera access, locking the phone or opening another app pauses only the built-in camera. The microphone, voice commands, Mac connection, and answers continue without images, and the camera restarts automatically when RayBridge returns. In the foreground this mode is not hands-free: the iPhone has to be pointed by hand.

English (US) **on-device** speech recognition is required for the voice prototype. If it is unavailable, typed questions still work. **Setup → Use iPhone audio for testing** explicitly allows phone audio when glasses are unavailable. Default operation requires a Bluetooth headset route. Bluetooth routing, SDK streaming, and speech recognition cannot be considered validated by a successful simulator build. RayBridge acknowledges each question immediately and announces progress about every 30 seconds while a task is still running. Long tasks continue until they finish, the user says Cancel, the session stops, or the connection is lost.

**Setup → Sound cues → Listen for voice commands** is on by default. When the app opens while stopped, it announces how to tap or say Start and how to hear command help. Say “commands” at any time to hear the available choices with a short pause between each command, including Status and Repeat. Say “start” to begin from hands-free standby, “stop” to end RayBridge, or “cancel” during thinking or speech to abandon the current turn and return to listening. “Status” reports whether the assistant is starting, working, cancelling, or idle. “Repeat” speaks the latest completed answer again. “Mute” lets the current turn and answer continue while listening only for “unmute” and “commands.” RayBridge immediately speaks a short confirmation for every recognized command. Status, Repeat, Mute, and Unmute briefly pause a playing Apple or Kokoro answer for the announcement, then resume it. Commands must be spoken as standalone words, which preserves questions such as “Where is the bus stop?” **Listen for Start while stopped** is also on by default and keeps the microphone active for Start and Commands while RayBridge is stopped, using the iPhone microphone when glasses audio is unavailable. Start RayBridge before locking the phone: an active Meta-glasses session continues listening, sending fresh images, and speaking answers while locked or while another app is open. Hands-free standby intentionally stops in the background. Cancel cannot undo computer actions Codex already completed. Echo suppression and command recognition must be validated on the actual glasses.
...
The large **Stop RayBridge** control stops the microphone, speech, camera, pending model answer, and Mac connection. It also cancels startup. Wait for camera teardown to finish before starting again. Each Start opens a new Mac conversation; previous transcript/answer text remains visible until you select **New conversation**. An active Meta-glasses session uses Meta's compressed HEVC stream and continues through lock and background transitions. Opening Meta AI for camera permission also preserves startup. An active iPhone session continues as audio-only while its camera is unavailable and restores camera capture on return. On a configuration where iOS reports multitasking camera access as supported, RayBridge enables it and keeps the iPhone camera running too.
...
The remaining work to achieve the original fully real-time experience is substantial: validate the physical iPhone/glasses loop, read the new stage timings from a real session, open the Claude process when the phone connects so the first question does not pay its startup, do the same for Hermes and shorten the fixed pause that ends dictation, and investigate a documented subscription-compatible real-time media interface if OpenAI provides one. This prototype does not claim that interface exists.
