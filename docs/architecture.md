...
Subscription sign-in is implemented through `account/login/start` with `type: chatgpt`. `account/read` must identify a ChatGPT account before inference. An API-key account is rejected. The bridge uses `thread/start`, `turn/start`, `turn/interrupt`, and final item/turn notifications. Model selection follows the account's default rather than hardcoding an API-only model.
